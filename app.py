...
# Официальное расписание звонков СибГИУ (sibsiu.ru/raspisanie/).
PAIR_TIMES = {
    1: "8:30–9:50", 2: "10:00–11:30", 3: "12:00–13:30", 4: "13:50–15:20",
    5: "15:30–17:00", 6: "17:10–18:40", 7: "18:00–19:30", 8: "19:35–21:05",
}


def pair_label(lesson: int) -> str:
    t = PAIR_TIMES.get(lesson)
    return f"{lesson} пара ({t})" if t else f"{lesson} пара"


def is_online_course(subject: str) -> bool:
    """A pre-recorded, asynchronous course (СУО/Moodle) rather than a live
    pair — no teacher, no room, and a fixed time slot means nothing here."""
    if not subject:
        return False
    s = subject.lower()
    return "курс" in s and ("суо" in s or "moodle" in s or "сибгиу" in s and "запис" in s)
...
def relevant_week_monday():
    """Monday of the week currently shown (see get_week_parity: on a weekend
    this is already next week, so students can check ahead)."""
    today = datetime.now().date()
    week_date = today + timedelta(days=(7 - today.weekday())) if today.weekday() >= 5 else today
    return week_date - timedelta(days=week_date.weekday())


def is_relevant_date(date_iso):
    """Master's/session-style schedules give every lesson its own specific
    calendar date (week_type='date') instead of a repeating odd/even slot,
    so a plain "show me Thursday's classes" query would otherwise return
    every Thursday of the whole semester at once. Keep only the occurrence
    that falls in the week currently being shown."""
    if not date_iso:
        return True
    try:
...
def format_teacher_results(rows, teacher, selected_day):
    rows=[r for r in rows if is_active(r.get('valid_from'), r.get('valid_until')) and is_relevant_date(r.get('date_iso'))]
    if not rows:
        return (
    ...
    for r in rows[:70]:
        ...
        tail=f"🔄 {'нечётная' if week=='odd' else 'чётная'}\n" if week in ("odd","even") else ""
        blocks.append(
...
def format_group_results(rows, group_name, selected_day):
    ...
    live_rows=[]
    for r in rows[:200]:
        if is_online_course(r.get('subject')):
            name=re.sub(r"\s*—\s*курс в записи.*$", "", r.get('subject') or "", flags=re.IGNORECASE).strip() or (r.get('subject') or "Курс")
    ...
    merged=[]
    for r in live_rows:
        key=(r['day'], r['lesson'], r.get('week_type'), r.get('subject'), r.get('date_text'))
        if merged and merged[-1]['_key']==key:
            ...
            merged.append({'_key':key, 'day':r['day'], 'lesson':r['lesson'], 'week_type':r.get('week_type'),
                           'subject':r.get('subject'), 'date_text':r.get('date_text'),
                           '_teachers':[r['teacher']] if r.get('teacher') else [],
                           '_rooms':[r['room']] if r.get('room') else []})

    if not merged and not online:
        if selected_day != "ALL":
            return (
                f"🎓 <b>{html.escape(group_name)}</b>\n"
                f"Неделя: <b>{week_title()}</b>\n"
    ...
    for slot in merged[:70]:
        ...
        if selected_day == "ALL" and day != last_day:
            blocks.append(f"\n📅 <b>{html.escape(day)}</b>")
            last_day = day
        pair = pair_label(slot['lesson'])
        subject = html.escape(slot.get("subject") or "Предмет не указан")
        week = slot.get("week_type")
        date_suffix = f" ({html.escape(slot['date_text'])})" if slot.get("date_text") else ""
        lines=[f"🕒 <b>{pair}</b> — {html.escape(day)}{date_suffix}\n📚 {subject}\n"]
        if slot['_teachers']:
            lines.append(f"👨‍🏫 {html.escape('; '.join(slot['_teachers']))}\n")
        if slot['_rooms']:
            lines.append(f"🚪 {html.escape(', '.join(slot['_rooms']))}\n")
        if week != "date":
            week_text = {"odd": "нечётная", "even": "чётная"}.get(week, week or "")
            lines.append(f"🔄 {html.escape(week_text)}\n")
        blocks.append("".join(lines))
    if len(merged) > 70:
        blocks.append(f"\n…и ещё {len(merged) - 70} записей")
    if online:
        blocks.append("\n📎 <b>Курсы в записи (СУО/Moodle, без очных пар):</b>")
        blocks.extend(f"• {html.escape(name)}" for name in online)
    if not blocks:
        blocks.append("📭 В этот день занятий нет.")
    return header + "\n".join(blocks)
