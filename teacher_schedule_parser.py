...
TRAILING_ROOM_RE = re.compile(
    r"\s*[-–—]?\s*\d{1,4}(?:\s*[А-ЯA-ZЁа-яё]{1,3})?(?:\s*,\s*\d{1,4}(?:\s*[А-ЯA-ZЁа-яё]{1,3})?)*\s*$"
)


def strip_trailing_rooms(text: str) -> str:
    ...
    return TRAILING_ROOM_RE.sub("", text)
...
def extract_entries(raw: str, filename: str = ""):
    ...
    if not teachers:
        ...
        subject=proj_subject or re.sub(r"\s+", " ", TITLE_RE.sub(" ", strip_trailing_rooms(cleaned))).strip(" /-")
    ...
    for i,m in enumerate(teachers):
        ...
        if subject is None:
            ...
            subject=strip_trailing_rooms(subject)
...
def build_records(root: str|Path):
    ...
    unique=[]; seen=set()
    for r in records:
        key=(r['institute'],r['source_file'],r['sheet'],r['degree'],r['course'],r['group'],r['day'],r['date_text'],r['lesson'],r['week_type'],r['subject'],r['teacher'],r['room'],r.get('valid_from'),r.get('valid_until'))
        if key not in seen:
...
def create_db(records, db_path):
    ...
    rows=[(r['institute'],r['source_file'],r['sheet'],r.get('degree'),r.get('course'),r['group'],r['day'],r['date_text'],r['lesson'],r['week_type'],r['subject'],r['teacher'],r['teacher_search'],r['room'],r.get('valid_from'),r.get('valid_until'),r['raw']) for r in records]
    cur.executemany('''INSERT INTO lessons(institute,source_file,sheet,degree,course,group_name,day,date_text,lesson,week_type,subject,teacher,teacher_search,room,valid_from,valid_until,raw)
                       VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)''',rows)
...
def search_teacher(query, day=None, week_type=None, db_path='schedule.db'):
    ...
    sql='''SELECT institute,degree,course,group_name,day,date_text,lesson,week_type,subject,teacher,room,valid_from,valid_until FROM lessons WHERE teacher_search LIKE ?'''
    params=[f'%{q}%']
    if day:
...
def search_group(group_name, institute=None, day=None, week_type=None, db_path='schedule.db'):
    ...
    schedules together."""
    con=sqlite3.connect(db_path); con.row_factory=sqlite3.Row; cur=con.cursor()
    sql='''SELECT institute,group_name,day,date_text,lesson,week_type,subject,teacher,room,valid_from,valid_until FROM lessons WHERE group_name=?'''
    params=[group_name]
    if institute:
