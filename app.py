...
def query_group_day(institute: str, group: str, day: str):
    parity = get_week_parity()
    con = db_connect()
    rows = con.execute(
        "SELECT institute, group_name, day, lesson, week_type, subject, teacher, room, raw "
        "FROM group_lessons "
        "WHERE institute=? AND group_name=? AND day=? AND week_type IN (?, 'date') "
        "ORDER BY lesson, id",
        (institute, group, day, parity)
    ).fetchall()
    con.close()
    return [dict(r) for r in rows]


def format_group_schedule(rows, group: str, institute: str, day: str):
    parity = get_week_parity()
    ...
    for lesson in sorted(blocks):
        ...
        if not entries:
            continue
        has_lessons = True
        lines.append(f"🕒 <b>{lesson} пара</b> ({PAIR_TIMES.get(lesson, '')})")
...
async def schedule_day(callback: types.CallbackQuery, state: FSMContext):
    ...
    if not institute or not group or day not in DAYS:
        ...
        return
    rows = query_group_day(institute, group, day)
    text = format_group_schedule(rows, group, institute, day)
    builder = InlineKeyboardBuilder()
