...
def format_teacher_results(rows, teacher, selected_day):
    ...
    for r in rows[:70]:
        ...
        active=r.get('active_periods') or []
        date_text=""
        if r.get('date_text'):
            ...
            for a,b in active:
                try:
                    af=a[8:10]+'.'+a[5:7]
                    bf=b[8:10]+'.'+b[5:7]
                    pretty.append(af if af==bf else f"{af}-{bf}")
                except Exception:
                    pass
            if pretty:
                date_text=f" | {html.escape(', '.join(pretty))}"
        elif dates:
            date_text=f" | {html.escape(', '.join(dates))}"

        # При выборе конкретного дня не дублируем название дня в каждой паре.
        if selected_day == "ALL":
            time_line=f"🕒 <b>{pair}</b> — {html.escape(day)}{date_text}"
        else:
            time_line=f"🕒 <b>{pair}</b>{date_text}"
...
async def teacher_day(callback: types.CallbackQuery, state: FSMContext):
    ...
    rows=search_teacher(teacher, day=day, week_type=get_teacher_week_parity(), as_of_date=datetime.now().date().isoformat(), db_path=TEACHER_DB)
...
@app.get("/")
def index():
    return {"status":"Bot is running 24/7!","teacher_records":teacher_record_count()}
