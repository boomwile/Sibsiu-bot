...
def build_records(root: str|Path):
    ...
    unique=[]; seen=set()
    for r in records:
        key=(r['institute'],r['source_file'],r['sheet'],r['degree'],r['course'],r['group'],r['day'],r['date_text'],r['lesson'],r['week_type'],r['subject'],r['teacher'],r['room'],tuple(r['date_ranges']))
        if key not in seen:
...
def create_db(records, db_path):
    ...
    rows=[(r['institute'],r['source_file'],r['sheet'],r.get('degree'),r.get('course'),r['group'],r['day'],r['date_text'],r['lesson'],r['week_type'],r['subject'],r['teacher'],r['teacher_search'],r['room'],json.dumps(r['date_ranges'],ensure_ascii=False),r['raw']) for r in records]
    cur.executemany('''INSERT INTO lessons(institute,source_file,sheet,degree,course,group_name,day,date_text,lesson,week_type,subject,teacher,teacher_search,room,date_ranges,raw)
                       VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)''',rows)
...
def search_group(group_name, institute=None, day=None, week_type=None, db_path='schedule.db'):
    ...
    schedules together."""
    con=sqlite3.connect(db_path); con.row_factory=sqlite3.Row; cur=con.cursor()
    sql='''SELECT institute,group_name,day,date_text,lesson,week_type,subject,teacher,room,date_ranges FROM lessons WHERE group_name=?'''
    params=[group_name]
    if institute:
