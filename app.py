...
# НЕДЕЛЯ
# =========================
def get_week_parity(for_date=None):
    """
    Определяет чётность учебной недели.
    В субботу и воскресенье автоматически берём следующую неделю,
    чтобы студент мог заранее посмотреть расписание.
    """
    now = for_date or datetime.now()
    if not isinstance(now, datetime):
        now = datetime.combine(now, datetime.min.time())

    # На выходных показываем уже следующую учебную неделю.
