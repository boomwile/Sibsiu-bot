...
SPACED_LETTERS_RE = re.compile(r"(?:[А-ЯЁA-Za-z][ \t]{5,}){2,}[А-ЯЁA-Za-z]")
...
def norm(v) -> str:
    ...
    # A few sheets spell one word out with a wide, decorative gap between
    # every letter (e.g. "М       А       Т       Е       М       А
    # Т       И       К       А" for "МАТЕМАТИКА"). That gap (~7 spaces) is
    # reliably wider than any normal word-spacing gap (1-3 spaces) elsewhere
    # in the same cell, so it can be told apart and collapsed *before* the
    # generic whitespace squeeze below erases that distinction.
    s = SPACED_LETTERS_RE.sub(lambda m: re.sub(r"\s+", "", m.group(0)), s)
    s = re.sub(r"[ \t]+", " ", s)
    s = re.sub(r"\n+", "\n", s)
    return s.strip()
...
def parse_day(v: str) -> tuple[Optional[str], Optional[str]]:
    ...
    # A handful of source files have a typo in the full day name - a stray
    # extra letter glued to the front ("ПЧЕТВЕРГ") or a truncated ending
    # ("СУББОТ"). Missing one of these silently makes every following row
    # inherit the *previous* day, corrupting a whole day's worth of lessons,
    # so it's worth a tolerant fuzzy match against the long day names (the
    # 2-letter abbreviations are deliberately excluded: they're too short to
    # fuzz-match safely).
    first_token = s.split()[0].strip(".,;:") if s.split() else s
    if 4 <= len(first_token) <= 14:
        for key, day in DAY_NAMES.items():
            if len(key) < 6:
                continue
            if key in first_token or (first_token in key and len(first_token) >= len(key)-2):
                dates = DATE_RE.findall(s)
                return day, (dates[0] if dates else None)
    return None, None
    ...
    while r<=end_row:
        ...
        for spec in specs:
            ...
            if date_based:
                ...
                assigns=[]
                for o in occ:
                    # A "до 21.10" / "с 11.11" qualifier (most often seen on
                    # sequential "Проектная деятельность 1/2" phases) means
                    # this occurrence is gated by calendar date, not by week
                    # parity - it is meant to recur every week within its own
                    # date window, not additionally be confined to only odd
                    # or only even weeks on top of that. Put it on both.
                    if (len(occ)==1 and o['start']<=lesson_start and o['end']>=end) or has_explicit_date(o['text']):
                        # The only content found in this slot spans the
                        # *entire* physical height of the block (no other row
                        # held anything different) -> one lesson rendered
                        # across several visual lines, not a genuine
                        # odd/even split, so it recurs every week.
                        assigns.append((o['text'],"odd")); assigns.append((o['text'],"even"))
                    else:
                        # The pair's cell is visually split into a top half
                        # (нечётная/odd week) and a bottom half (чётная/even
                        # week). When the block's total height is odd, the
                        # extra row goes to the top half (matches every such
                        # block found in the source files, e.g. a 3-row block
                        # splits 2 top / 1 bottom).
                        mid=(o['start']+o['end'])/2
...
def has_explicit_date(text: str) -> bool:
    """Quick check for a calendar-date marker (a "до/с/по DD.MM" qualifier,
    or an explicit "DD.MM-DD.MM" range) inside a lesson's text, without
    doing the full extraction. Used to tell a genuine odd/even week split
    apart from two sequential date-gated phases of the same discipline
    (e.g. "Проектная деятельность 1 до 21.10" / "2 с 11.11"), which must
    each recur on *both* weeks within their own date window rather than
    being additionally confined to only one parity."""
    return bool(DATE_RANGE_RE.search(text) or DATE_QUALIFIER_RE.search(text))
